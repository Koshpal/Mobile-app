...
object BankSmsUtils {
  ...
  private val BANK_SENDER_PATTERNS =
          listOf(
  ...
  data class TransactionInfo(val amount: String, val type: String, val rawMessage: String)

  /**
   * Check if a given SMS is likely a bank SMS based on sender, message content, and account
   * numbers.
   */
  ...
  fun extractTransactionInfo(message: String): TransactionInfo? {
    try {
      ...
      return TransactionInfo(amount = amount, type = type, rawMessage = message)
    } catch (e: Exception) {
  ...
  fun extractAccountNumbers(message: String): List<String> {
    return ACCOUNT_NUMBER_PATTERN.findAll(message).map { it.value }.toList()
  }
}
